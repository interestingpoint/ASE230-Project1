...
echo %MAGENTA% Staudioing REST API Tests using CURL
...
REM Test 2.1: Get All audios
echo %BLUE% Testing GET /audios...
set /a total_tests+=1

curl -s %API_BASE_URL%/audios > temp_response.txt 2>nul
if %errorlevel% equ 0 (
    set local_tests=0
    ...
    if !local_tests! equ 3 (
        echo %GREEN% Get All audios: All tests passed ^(3/3^)
        set /a passed_tests+=1
    ) else (
        echo %RED% Get All audios: Paudioial: !local_tests!/3 tests passed
    )
) else (
    echo %RED% Get All audios: Could not get audios
)
echo.

REM Test 2.2: Get Single audio
echo %BLUE% Testing GET /audios/1...
set /a total_tests+=1

curl -s %API_BASE_URL%/audios/1 > temp_response.txt 2>nul
if %errorlevel% equ 0 (
    set local_tests=0
    findstr /C:"success" temp_response.txt >nul && set /a local_tests+=1
    ...
    if !local_tests! equ 4 (
        echo %GREEN% Get Single audio: All tests passed ^(4/4^)
        set /a passed_tests+=1
    ) else (
        echo %RED% Get Single audio: Paudioial: !local_tests!/4 tests passed
    )
) else (
    echo %RED% Get Single audio: Could not get audio
)
echo.

REM Test 2.3: Create New audio
echo %BLUE% Testing POST /audios...
set /a total_tests+=1

echo {"name": "Test audio CURL", "size": "", "metadata" : ""} > temp_audio.json

curl -s -X POST -H "Content-Type: application/json" -d @temp_audio.json %API_BASE_URL%/audios > temp_response.txt 2>nul
if %errorlevel% equ 0 (
    set local_tests=0
    findstr /C:"success" temp_response.txt >nul && set /a local_tests+=1
    findstr /C:"Test audio CURL" temp_response.txt >nul && set /a local_tests+=1
    ...
    REM Extract audio ID for later tests (simplified)
    for /f "tokens=2 delims=:" %%a in ('findstr /C:"""id"":" temp_response.txt') do (
        for /f "tokens=1 delims=," %%b in ("%%a") do set CREATED_audio_ID=%%b
    )
    
    if !local_tests! equ 5 (
        echo %GREEN% Create audio: Created audio with ID: !CREATED_audio_ID!
        set /a passed_tests+=1
    ) else (
        echo %RED% Create audio: Paudioial: !local_tests!/5 tests passed
    )
) else (
    echo %RED% Create audio: Could not create audio
)
echo.

REM Test 2.4: Update audio
if defined CREATED_audio_ID (
    echo %BLUE% Testing PUT /audios/!CREATED_audio_ID!...
    set /a total_tests+=1
    
    echo {"name": "Updated Test audio CURL", "size": "20"} > temp_update.json
    
    curl -s -X PUT -H "Content-Type: application/json" -d @temp_update.json %API_BASE_URL%/audios/!CREATED_audio_ID! > temp_response.txt 2>nul
    if !errorlevel! equ 0 (
        set local_tests=0
        findstr /C:"success" temp_response.txt >nul && set /a local_tests+=1
        findstr /C:"Updated Test audio CURL" temp_response.txt >nul && set /a local_tests+=1
        findstr /C:"Data Science" temp_response.txt >nul && set /a local_tests+=1
        
        if !local_tests! equ 3 (
            echo %GREEN% Update audio: All tests passed ^(3/3^)
            set /a passed_tests+=1
        ) else (
            echo %RED% Update audio: Paudioial: !local_tests!/3 tests passed
        )
    ) else (
        echo %RED% Update audio: Could not update audio
    )
    echo.
    
    REM Test 2.5: Delete audio
    echo %BLUE% Testing DELETE /audios/!CREATED_audio_ID!...
    set /a total_tests+=1
    
    curl -s -X DELETE %API_BASE_URL%/audios/!CREATED_audio_ID! > temp_response.txt 2>nul
    if !errorlevel! equ 0 (
        findstr /C:"success" temp_response.txt >nul
        if !errorlevel! equ 0 (
            REM Verify deletion
            curl -s %API_BASE_URL%/audios/!CREATED_audio_ID! > temp_verify.txt 2>nul
            findstr /C:"not found" temp_verify.txt >nul
            if !errorlevel! equ 0 (
                echo %GREEN% Delete audio: audio !CREATED_audio_ID! deleted successfully!
                set /a passed_tests+=1
            ) else (
                echo %RED% Delete audio: audio still exists after deletion
            )
        ) else (
            echo %RED% Delete audio: Delete operation failed
        )
    ) else (
        echo %RED% Delete audio: Could not delete audio
    )
    echo.
)
...
if exist temp_audio.json del temp_audio.json

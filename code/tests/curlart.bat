...
REM Test 2.1: Get All arts
echo %BLUE% Testing GET /arts...
set /a total_tests+=1

curl -s %API_BASE_URL%/arts > temp_response.txt 2>nul
if %errorlevel% equ 0 (
    set local_tests=0
    ...
    if !local_tests! equ 3 (
        echo %GREEN% Get All arts: All tests passed ^(3/3^)
        set /a passed_tests+=1
    ) else (
        echo %RED% Get All arts: Partial: !local_tests!/3 tests passed
    )
) else (
    echo %RED% Get All arts: Could not get arts
)
echo.

REM Test 2.2: Get Single art
echo %BLUE% Testing GET /arts/1...
set /a total_tests+=1

curl -s %API_BASE_URL%/arts/1 > temp_response.txt 2>nul
if %errorlevel% equ 0 (
    set local_tests=0
    findstr /C:"success" temp_response.txt >nul && set /a local_tests+=1
    ...
    if !local_tests! equ 4 (
        echo %GREEN% Get Single art: All tests passed ^(4/4^)
        set /a passed_tests+=1
    ) else (
        echo %RED% Get Single art: Partial: !local_tests!/4 tests passed
    )
) else (
    echo %RED% Get Single art: Could not get art
)
echo.

REM Test 2.3: Create New art
echo %BLUE% Testing POST /arts...
set /a total_tests+=1

echo {"name": "Test art CURL", "size": "", "artist" : ""} > temp_art.json

curl -s -X POST -H "Content-Type: application/json" -d @temp_art.json %API_BASE_URL%/arts > temp_response.txt 2>nul
if %errorlevel% equ 0 (
    set local_tests=0
    findstr /C:"success" temp_response.txt >nul && set /a local_tests+=1
    findstr /C:"Test art CURL" temp_response.txt >nul && set /a local_tests+=1
    ...
    REM Extract art ID for later tests (simplified)
    for /f "tokens=2 delims=:" %%a in ('findstr /C:"""id"":" temp_response.txt') do (
        for /f "tokens=1 delims=," %%b in ("%%a") do set CREATED_art_ID=%%b
    )
    
    if !local_tests! equ 5 (
        echo %GREEN% Create art: Created art with ID: !CREATED_art_ID!
        set /a passed_tests+=1
    ) else (
        echo %RED% Create art: Partial: !local_tests!/5 tests passed
    )
) else (
    echo %RED% Create art: Could not create art
)
echo.

REM Test 2.4: Update art
if defined CREATED_art_ID (
    echo %BLUE% Testing PUT /arts/!CREATED_art_ID!...
    set /a total_tests+=1
    
    echo {"name": "Updated Test art CURL", "size": "20"} > temp_update.json
    
    curl -s -X PUT -H "Content-Type: application/json" -d @temp_update.json %API_BASE_URL%/arts/!CREATED_art_ID! > temp_response.txt 2>nul
    if !errorlevel! equ 0 (
        set local_tests=0
        findstr /C:"success" temp_response.txt >nul && set /a local_tests+=1
        findstr /C:"Updated Test art CURL" temp_response.txt >nul && set /a local_tests+=1
        findstr /C:"Data Science" temp_response.txt >nul && set /a local_tests+=1
        
        if !local_tests! equ 3 (
            echo %GREEN% Update art: All tests passed ^(3/3^)
            set /a passed_tests+=1
        ) else (
            echo %RED% Update art: Partial: !local_tests!/3 tests passed
        )
    ) else (
        echo %RED% Update art: Could not update art
    )
    echo.
    
    REM Test 2.5: Delete art
    echo %BLUE% Testing DELETE /arts/!CREATED_art_ID!...
    set /a total_tests+=1
    
    curl -s -X DELETE %API_BASE_URL%/arts/!CREATED_art_ID! > temp_response.txt 2>nul
    if !errorlevel! equ 0 (
        findstr /C:"success" temp_response.txt >nul
        if !errorlevel! equ 0 (
            REM Verify deletion
            curl -s %API_BASE_URL%/arts/!CREATED_art_ID! > temp_verify.txt 2>nul
            findstr /C:"not found" temp_verify.txt >nul
            if !errorlevel! equ 0 (
                echo %GREEN% Delete art: art !CREATED_art_ID! deleted successfully!
                set /a passed_tests+=1
            ) else (
                echo %RED% Delete art: art still exists after deletion
            )
        ) else (
            echo %RED% Delete art: Delete operation failed
        )
    ) else (
        echo %RED% Delete art: Could not delete art
    )
    echo.
)
...
if exist temp_art.json del temp_art.json
